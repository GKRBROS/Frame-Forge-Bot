...
export const SafeMarkdown: React.FC<SafeMarkdownProps> = ({ content, stream }) => {
  const [modules, setModules] = useState<{
    ReactMarkdown: any;
    remarkGfm: any;
  } | null>(null);

  const [displayedContent, setDisplayedContent] = useState(stream ? "" : content);
  ...
  return (
    <ReactMarkdown
      ...
      components={{
        h1: ({ children }: any) => <h1 className="text-xl font-semibold mt-2 mb-1">{children}</h1>,
        ...
        blockquote: ({ children }: any) => <blockquote className="border-l-2 border-primary/40 pl-3 italic text-muted-foreground">{children}</blockquote>,
      }}
    >
      {displayedContent}
    </ReactMarkdown>
